...
class link_vuetify_generic:
    ...
    def __init__(self, event, widget, state, attribute,
                 function_to_widget=None, function_to_state=None):
        self.widget = ref(widget)
        self.state = ref(state)
        self.attribute = attribute
        self.function_to_widget = (ref(function_to_widget)
                                   if function_to_widget is not None else None)
        self.function_to_state = (ref(function_to_state)
                                  if function_to_state is not None else None)
        self.update_widget()
    ...
    def update_state(self, *ignore_args):
        ...
        if self.function_to_state:
            value = self.function_to_state()(self.widget().v_model)
            if getattr(self.state(), self.attribute) == value:
                return
            setattr(self.state(), self.attribute, value)
        else:
