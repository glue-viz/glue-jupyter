...
class Volume3DLayerStateWidget(VBox):
    ...
    def __init__(self, layer_state):
        ...
        self.widget_render_method = Dropdown(options=render_methods, value=self.state.render_method, description='method')
        ...
        self.widget_opacity = FloatSlider(description='opacity', min=0, max=1, value=self.state.alpha, step=0.001)
        link((self.state, 'alpha'), (self.widget_opacity, 'value'))

        self.widget_opacity_scale = FloatLogSlider(description='opacity scale', base=10, min=-3, max=3, value=self.state.opacity_scale, step=0.01)
